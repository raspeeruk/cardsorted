...
export async function POST(req: NextRequest) {
  ...
  if (!RESEND_KEY) {
    console.warn("CardSorted contact form: RESEND key not configured");
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "CardSorted <noreply@siftforms.com>",
      to: NOTIFY_EMAIL,
      reply_to: email,
      subject: `[CardSorted] ${topic}: ${name}`,
      text: lines.join("\n"),
    }),
  });
  ...
  if (!res.ok) {
    ...
    console.error("CardSorted contact form Resend error:", res.status, txt);
